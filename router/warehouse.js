 ...
 * @access public
 */
warehouseRoute.get("/", async (req, res) => {
  try {
    const pageSize = req.query.pageSize ? parseInt(req.query.pageSize) : 0;
    const page = req.query.page ? parseInt(req.query.page) : 0;
    const { keyword, province, district, sortBy, name, phone, street, ward } =
      req.query;
    var query = {};
    ...
    const warehouses = await Warehouse.find({ $and: [query, keywordCondition] })
      .limit(pageSize)
      .skip(pageSize * page)
      .sort(`${sortBy}`);
    const length = await Warehouse.find({
    ...
    if (warehouses)
      return sendSuccess(res, "Get warehouse successfully.", {
        length,
        warehouses,
      });
    return sendError(res, "No warehouse found.");
 ...
 * @access public
 */
warehouseRoute.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
  ...
  async (req, res) => {
    try {
      ...
      if (warehouse.storekeeper == req.user.role._id) {
        ...
        const productShipment = await ProductShipment.findById(
          productShipmentId
        );
        if (!productShipment)
          return sendError(res, "The product shipment not found.");
        ...
        if (product_shipments.length == 0) {
          ...
          let inventory_product_shipments = [
            ...warehouse.inventory_product_shipments,
            add,
          ];
          await Warehouse.findByIdAndUpdate(warehouseId, {
            inventory_product_shipments: inventory_product_shipments,
...
warehouseRoute.put(
  ...
  async (req, res) => {
    ...
    const warehouse = await Warehouse.findById(warehouseId);
    if (!warehouse) {
      return sendError(res, "The warehouse not found.");
    }
    if (warehouse.storekeeper == req.user.role._id) {
      try {
        ...
        for (let i = 0; i < warehouse.inventory_product_shipments.length; i++) {
          if (
            warehouse.inventory_product_shipments[i].shipment ==
            productShipmentId
          ) {
            ...
            warehouse.turnover =
              warehouse.turnover +
              (status == "import"
                ? +warehouse.inventory_product_shipments[i].turnover
                : -warehouse.inventory_product_shipments[i].turnover);
            await Warehouse.findByIdAndUpdate(warehouseId, {
        ...
        return sendError(
          res,
          "This product shipment can not be found in this warehouse."
        );
      } catch (error) {
        return sendServerError(res);
      }
